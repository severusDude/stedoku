		...
		for (let s = 0; s < 9 && !found; s++) {
			for (let w = 0; w < 9 && !found; w++) {
				for (let c = 0; c < 9 && !found; c++) {
					if (reference[s][w][c].toLowerCase() === ch.toLowerCase()) {
...
const secret = "Kelompok 3 KI";

// Step 1: Preprocess the cover text
const referenceMatrix = preprocessCoverText(coverText);

console.log(referenceMatrix);

// Step 2: Encode secret message
const sudokuBoard = encodeSecretText(secret, referenceMatrix);

// Step 3: Decode it back
const recovered = decodeBoard(sudokuBoard, referenceMatrix);

console.log("Encoded Sudoku:\n", sudokuBoard);
